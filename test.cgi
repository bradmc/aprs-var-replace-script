...
{
	my $content = shift;
	while ($content =~ s/aprs\(\s*IF\s*(.*?)\s+(.*?)\s+(.*?)\s*THEN\s*\"(.*?)\"\s*ELSE\s*\"(.*?)\"\s*\)/aprs(TOKEN)/)
	{
		my $test = resolve_if_function($1,$2,$3,$4,$5);
		$content =~ s/aprs\(TOKEN\)/$test/;
	}
	return $content;
}

sub resolve_if_function
{
	my ($left,$oper,$right,$then,$else) = @_;
	if ($oper eq '<') { if ($left < $right) { return $then } else { return $else } }
}
